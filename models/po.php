 ...
 * HelloWorld Model
*/
class StalramsModelPo extends JModelLegacy
{
    ...
    function getlog()
	{
		...
		$msg = JRequest::getVar('log');
		if ($user->name == '') $userlog = 'Не определен';
		else $userlog = $user->name;
	
		$db = JFactory::getDbo();
		$db->setQuery("INSERT INTO #__neq_log (username, msg) VALUES ('".$userlog."','".$msg."')");
		$db->query();
	}
 
	function getPO() {//Получаем список производственных отделений в регионах
		
		$region = JRequest::getVar('region');//Пределяем опорный регион

		$db = JFactory::getDBO();// Подключаемся к базе.
		$query = "SELECT code AS value, name AS text FROM #__neq_pes WHERE regid = ".$region." ORDER BY text";//Определяем запрос
		//echo $query;
	...
	function getObjectPO() {//Получаем список географических объектов в регионе в соответствующем ПО соответствующего типа (ПС, РЭС и т.д.)
	
		$base = JRequest::getVar('base');
		$pes = JRequest::getVar('pes');
		//echo $pes.' '.$base; 
		$db = JFactory::getDBO();// Подключаемся к базе.
		$query = "SELECT #__neq_po.name AS text, #__neq_po.number AS value, #__neq_po.id AS id FROM #__neq_po
				  INNER JOIN #__neq_base ON #__neq_po.basetype = #__neq_base.id	INNER JOIN #__neq_pes ON #__neq_po.pes = #__neq_pes.id
				  WHERE #__neq_base.code = '".$pes."'
					AND #__neq_pes.code = '".$base."'
				  GROUP BY #__neq_po.name";
		//echo $query;
		$db->setQuery($query);//Выполняем запрос
	...
	function getObjcode() {//Получаем список производственных отделений в регионах
	
		$region = JRequest::getVar('type');//Пределяем опорный регион
	
		$db = JFactory::getDBO();// Подключаемся к базе.
		$query = "SELECT #__neq_category.name AS text
		          FROM #__neq_type
		          INNER JOIN #__neq_category ON #__neq_type.catcode = #__neq_category.id
				  WHERE toh1w_neq_type.code = '".$type."'";
		$db->setQuery($query);//Выполняем запрос
	...
	function getAppar() {//Получаем список аппаратных на объекте
	
		$idpo = JRequest::getVar('idpo');
		//echo $pes.' '.$base;
		$db = JFactory::getDBO();// Подключаемся к базе.
		$query = "SELECT  #__neq_a.name AS name, #__neq_a.code AS value FROM #__neq_a
				  WHERE #__neq_a.codepo = '".$idpo."'
				  ORDER BY name";
		//echo $query;
		$db->setQuery($query);//Выполняем запрос
    ...
    /**
     * Получаем список оборудования на объекте
     * @return string
     */
    function getZabbixObjects() {
		...
		try {
			...
			$rList = $api->hostGet(array('output' => 'extend','search' => array('host' => JRequest::getVar('object')), 'sortfield'=> 'host','selectInventory' => array('chassis')));
			
			$msg = '  Найдено : '.count($rList);
			$msg .= '<div style = "width: 900px; overflow: auto; height: 300px; background-color: white;">';
			$msg .= '<table>
						<thead>
							<tr>
								<th width="50">id</th>
								...
								<th width="300">Производитель</th>
							</tr>
						</thead>
						<tbody>';
			foreach($rList as $graph){
				$msg .= '<tr><td>' .$graph->hostid .'</td><td>'. $graph->host .'</td><td>'. $graph->name .'</td><td>'.$graph->inventory->chassis .'</td></tr>';
			}
			$msg .= '</tbody></table><div>';
			return $msg;
		} catch(Exception $e) {
